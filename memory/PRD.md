...
- [x] **Public 1-year statement now includes a proper accounting ledger table** matching the operator's requested layout: **Sl No · Date · Particulars · Name · Credit · Debit · Balance · Penalty (Yes/No)**. One row per bill raised (from `PeoplesAmountDetails`), resolving the source (Festival / Sub Tariff month / Marriage / Death) for the Name column. Backend returns `ledger` + `ledger_totals`. Frontend renders as a wide-card `<table>` with green/red styling and a Total row. WhatsApp message uses the same columns via a monospace ``` ` ``` ` `` code block. Live-verified via preview URL — table renders exactly like the reference screenshot.
