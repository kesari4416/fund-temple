...
- [x] **TC_COLLECTION_001 — Collection receipt WhatsApp** now sends a clean payment receipt (thank-you · amount · purpose · date · receipt # · payment mode · temple sign-off) instead of dumping the full 1-year ledger, ledger totals, and public link. Implemented via a new `receiptOnly` prop on `WhatsappStatementButton`; `Bill.jsx` (post-add print modal) now passes `receiptOnly` so the auto-share message stays concise. The full-ledger share is still available from Family Details → Balance Sheet (TC_FAMILYDETAILS_001).
