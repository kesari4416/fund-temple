...
- [x] **Public WhatsApp Member Statement — Management Interest & Chit Interest coverage.**
  Backend `collection/public_views.py::public_member_statement` was previously filtering `CollectionDetails.filter(member=member)`, so interest-type collection rows (which store the interest link but often leave `member` NULL) were invisible on the shared 1-year statement. Fixed:
  - **Collections query** now `filter(pay_date__gte=since, action=True).filter(Q(member=member) | Q(interest__people_member=member)).distinct()` — pulls both direct collections and rows where the linked `PeopleInterestDetails.people_member` is this member.
  - **Amount attribution** for interest rows now uses `principal + interst_amount + penalty_amount` (interest collections split these across 3 columns). Response includes `principal_amount`, `interest_amount`, `penalty_amount` so the UI can show the breakdown.
  - **Pending dues** now merges `PeoplesAmountDetails` (festival/tariff/marriage/death) with `PeopleInterestBalanceSheet.filter(interest__people_member=member, paid=False)` per interest_type ("Management Interest" and "Chit Interest" labels). Penalty balance is added to the outstanding.
  - Frontend `PublicMemberStatement.jsx` renders an extra `P / I / Pen` line beneath any interest row (`data-testid="statement-payment-{id}-breakdown"`).
