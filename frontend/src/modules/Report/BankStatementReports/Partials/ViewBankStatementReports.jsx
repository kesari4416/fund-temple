...
export const ViewBankStatementReports = () => {
    ...
    const DateSearch = (values) => {
        setIsLoadspin(true)
        request.post(`${APIURLS.BANK_STATEMENT_FILTER_REPORTS}`, values)
            .then(function (response) {
                setDataSource(response.data?.cash_det);
                if (response.data?.cash_det?.length) {
                    toast.success(
                        "Bank Statement report filtered by date successfully retrieved."
                    );
                } else {
                    toast.warn(
                        "No Bank Statement report data found for the selected date !"
                    );
                }
                setIsLoadspin(false)
                console.log(response.data, 'sdfsdfsdfdsfsdfsdf');
            })
            .catch(function (error) {
                toast.error("Failed");
                setIsLoadspin(false)
    ...
    const columns = [
        ...
        {
            title: "From",
            dataIndex: "bank_name2",
            render: (record, data) => {
                return <span style={{ display: 'flex', justifyContent: 'center' }}>{data?.type_choice === "Bank Transfer" ? data?.bank_name2 : '--'}</span>
            }
        },
        {
            title: "To",
            dataIndex: "bank_name",
            render: (record, data) => {
                return <span style={{ display: 'flex', justifyContent: 'center' }}>{data?.type_choice === "Bank Transfer" ? data?.bank_name : '--'}</span>
            }
        },
        ...
        <Fragment>
            ...
            >
                <CustomRow space={[24, 24]}>
                    <Col span={24} md={12}>
                        <CustomPageTitle Heading={"Bank Statement Report"} />
                    </Col>
                    <Col span={24} md={12}>
                        <Flex end={true}>
                            <Button.Secondary
