...
export const ViewDeathReports = () => {
  ...
  //---------------
  const SearchOptions = [
    { label: "Death Tariff", value: "Death Tariff" },
    { label: "Date Range", value: "Date Range" },
  ];

  //============ Death Tariff options ============

  const DeathTariffOptions = deathData?.map((deth) => ({
    label: `${deth?.member_name} / ${deth?.death_no}`,
    value: deth?.id,
  }));
  ...
  const handleSearchChange = (value) => {
    form.resetFields(["range", "death_name"]);
    setDataSource([]);

    if (value === "Death Tariff") {
      setDeathShow(true);
      setDateRange(false);
    ...
    request.post(`${APIURLS.TYPE_DATE_FILTER_REPORTS}`, values)
      .then(function (response) {
        ...
        if (response.data?.amount?.length) {
          toast.success(
            "Death Tariff report filtered by date successfully retrieved."
          );
        } else {
          toast.warn(
            "No Death Tariff report data found for the selected date !"
          );
        }
        setIsLoadspin(false)
      })
  ...
  const DeathTariffSearch = (values) => {
    setIsLoadspin(true)
    request.post(`${APIURLS.TYPE_LIST_FILTER_REPORTS}`, values)
      .then(function (response) {
        ...
        if (response.data?.amount?.length) {
          toast.success(
            "Death Tariff list filter reports successfully loaded."
          );
        } else {
          toast.warn("No data found for Death Tariff list filter reports !");
        }
        setIsLoadspin(false)
      })
      .catch(function (error) {
  ...
  const onFinish = (values) => {
    const deathValues = {
      type: values?.death_name,
      category: values?.category,
    };
    const DateRangeValues = {
      range: rangeFilterDate,
      category: "Death Tariff",
    };
    if (deathShow) {
      DeathTariffSearch(deathValues);
    } else {
    ...
    <Fragment>
      ...
      >
        <CustomRow space={[24, 24]}>
          <Col span={24} md={12}>
            <CustomPageTitle Heading={"Death Tariff Unpaid Report"} />
          </Col>
          <Col span={24} md={12}>
            <Flex end={true}>
              <Button.Secondary
                  ...
                  <>
                    <CustomRow space={[12, 24]}>
                      ...
                      <Col span={24} md={24} lg={8}>
                        <CustomSelect
                          name={"death_name"}
                          placeholder={"Please Select Death Tariff"}
                          options={DeathTariffOptions}
                          rules={[
                            { required: true, message: "Please Select a  Death Tariff !" },
                          ]}
                        />
                      </Col>
                      <Col span={24} md={24} lg={6}>
