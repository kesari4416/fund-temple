...
import { getExpenseName, getExpenseNameStatus, selectExpenseNameDetails } from "@modules/ExpenseDetails/ExpenseSlice";
import { AddExpenseNameModal } from "@modules/ExpenseDetails/AddExpense/Partials/AllExpenseModals";
...
const ExpenseNameList = () => {
    ...
    const AllExpenseNameDetails = useSelector(selectExpenseNameDetails);
    const ExpenseNameDetails = useSelector(getExpenseNameStatus);

    useEffect(() => {
        setDataSource(AllExpenseNameDetails);
    }, [AllExpenseNameDetails]);

    const EditExpenseName = (record) => {
        setModelwith(500);
        setTrigger(trigger + 1);
        setModalContent(
            <AddExpenseNameModal ExpenseNameRecord={record}
                FormUpdateClose={FormUpdateClose} ExpenseNameTrigger={trigger} />
        );
        showModal();
    };

    const DeleteExpenseName = async (data) => {
        await request.delete(`${APIURLS.UPDATE_EXPENSE_NAME}/${data?.id}/`, data)
            ...
            render: (text, record, index) => {
                return (
                    <Flex gap={"20px"} center={"true"}>
                        {superUsers || role === userRolesConfig.ADMIN || ExpensePermission?.Expense?.Edit ? (
                            <Tooltip title="Edit">
                                <TableIconHolder size={"28px"} onClick={() => EditExpenseName(record)}>
                                    <img src={SvgIcons.Edit} style={{ cursor: "pointer" }} />
                                </TableIconHolder>
                            </Tooltip>
                        ) : null}
                        {superUsers || role === userRolesConfig.ADMIN || ExpensePermission?.Expense?.Delete ? (
                            <CustomPopconfirm
                                title="Confirmation"
                                description="Are you sure you want to remove this expense detail?"
                                okText="Yes"
                                cancelText="No"
                                confirm={() => DeleteExpenseName(record)}
                            >
    ...
    if (ExpenseNameDetails === "loading") {
        content = <CommonLoading />;
    } else if (ExpenseNameDetails === "succeeded") {
    ...
    } else if (ExpenseNameDetails === "failed") {
        const rowKey = (dataSource) => dataSource.id;
        content = (
            <CustomStandardTable
                columns={TableColumn}
        ...
        <Fragment>
            <CustomCardView>
                <CustomRow space={[12, 12]}>
                    <Col span={24} md={12}>
                        <CustomPageTitle Heading={"Expense Name List"} />
                    </Col>
                    <Col span={24} md={12}>
                        <Flex aligncenter gap={"15px"}>
                            <p>Filter By Expense Name :&nbsp;</p>
                            <CustomInput
                                value={search2Texts}
                                placeholder="Search"
                ...
                <PrintHolder ref={componentRef}>
                    <PrintShowData className="PrintShowDatadd">
                        <h1 style={{ textAlign: 'center' }}>Expense Name Details</h1><br />
