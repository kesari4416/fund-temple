...
import { getAssetCategory, getAssetCategoryStatus, selectAssetCategoryDetails } from '../AssetSlice';
...
export const AssetCategoryList = () => {
    ...
    const AllDatas = useSelector(selectAssetCategoryDetails)
    const AllStatus = useSelector(getAssetCategoryStatus)

    const UpdateAssetsCategory = (record) => {
        setModelwith(600);
        setModalTitle('Update Asset Category')
        setTrigger(trigger + 1);
        setModalContent(<AssetModals updateRecord={record} FormUpdate={FormUpdate} UpdateTrigger={trigger} />);
        showModal();
    };

    const DeleteAsset = async (record) => {
        await request.delete(`${APIURLS.EDIT_ASSET_CATEGORY}/${record?.id}/`, record)
            ...
            render: (text, record, index) => {
                return (
                    <Flex gap={"20px"} center={"true"}>
                        ...
                        <CustomPopconfirm
                            title="Confirmation"
                            description="Are you sure you want to remove this asset category ?"
                            okText="Yes"
                            cancelText="No"
    ...
    return (
        <CustomCardView>
            <Flex aligncenter spacebetween maxWidthEnter={''}>
                <div>
                    <CustomPageTitle Heading={'Asset Category List'} />
                </div>
                <Flex aligncenter>
