...
import { getAssetCategory } from "../AssetSlice";
import { useDispatch } from "react-redux";
import { toast } from "react-toastify";

export const AssetModals = ({ close, updateRecord, propertytrigger, FormUpdate, UpdateTrigger }) => {
  ...
  const AssetCategory = async (data) => {
    await request.post(`${APIURLS.POST_Asset_Category}`, data)
  ...
  const AssetUpdateCategory = async (data) => {
    await request.put(`${APIURLS.EDIT_ASSET_CATEGORY}/${updateRecord?.id}/`, data)
      .then(function (response) {
        if (response.status === 226) {
          toast.warn(response.data.message)
        }
    ...
    >
      <CustomRow space={[24, 24]}>
        <Col span={24} md={24}>
          ...
          />
          {updateRecord ? <CustomInput name={'id'} display={'none'} /> : null}
        </Col>
      </CustomRow>
      ...
      {updateRecord ? <Flex center gap={"20px"} style={{ margin: "30px" }}>
        ...
        <Flex center gap={"20px"} style={{ margin: "30px" }}>
          ...
          <Button.Success text={"Cancel"} onClick={() => close()} />
        </Flex>
      }
